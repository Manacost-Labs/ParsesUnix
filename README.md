...
### Неизвестная стоимость никогда не равна нулю

```
Cost.free()     — платного вызова не было. Измеренный ноль.
Cost.of("5")    — провайдер назвал цену.
Cost.unknown()  — вызов был, цена неизвестна.
```

Третье состояние существует потому, что записанный ноль занижает счёт, а
занижённый счёт — это и есть механизм тихого перерасхода. `unknown` доходит до
отчёта неизменным и останавливает дальнейшие траты.
...
- **soak-тест на 10k+ URL** с рестартами и конкурентностью;
- **canary-система для бесплатного слоя** (платная — готова);
...
- исполнение фаз A–D раннером: фазы моделируются в оценке стоимости, но
  `Runner` пока проходит очередь одним проходом;
